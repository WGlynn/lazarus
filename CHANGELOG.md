...
- **The concrete edit now threads through the whole pipeline.** A `{file, find, replace}` edit is carried from the judge verdict all the way to the applied file: the judge's structured-output schema requests one on every would-change verdict, and it flows `Verdict → RetroFix → RetroFix.as_dict() → PendingFinding.fix → apply_fix`. Previously the judge emitted no `edit`, so auto-apply was a no-op on real fixes — the wire existed but nothing rode it. A single `apply.normalize_edit` defines "concrete vs advisory" for both the judge and the v1 engine so they can't disagree. The offline stub judge now emits real edits, the runnable demo applies them to (a copy of) a committed target file and `lazarus undo`s them, and `tests/test_autoapply_e2e.py` proves the full wire end-to-end with no API key.
...
### Fixed
- **`lazarus undo` could fail to fully restore a file that got more than one edit in the same clock tick.** Backups were keyed by millisecond timestamp; on coarse (≈16 ms) clocks, two auto-applied edits to the same file collided on the backup name, so the second write clobbered the first and undo could only revert one. Backup stamps are now bumped to the next free integer, keeping each backup distinct and application-ordered so undo is reliably LIFO. Regression-tested (`test_two_edits_same_file_undo_both_restores_original`).
