...
wraps the three organs of the tool:
...
Design contract (see the repo README):
  ...
  * Fail-loud on missing input. A missing config, a missing corpus, an empty
    ...
    visible stderr message and a non-zero exit. There is no silent no-op.
  ...
  * Proposes, never auto-applies. Nothing in this CLI writes to the corpus or
    ...
    action recorded via `lazarus ledger action`.
...
actually runs, so `sonar` and `ledger` work with zero third-party dependencies
and offline.
"""
...
# calls the model, so importing this CLI never requires the optional extra.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.set_defaults(_root_parser=parser, _ledger_root=ledger_parser)
...
def _dispatch(
    ...
    *,
    ...
    All fail-loud paths funnel through here: config, input, judge, and ledger
    errors each print a visible stderr message and return their dedicated exit
    code. This keeps every handler free of exit-code plumbing and guarantees the
    CLI never silently no-ops on a failure.
    """
    handler = getattr(args, "_handler", None)
    if handler is None:
        # No subcommand (or a bare `ledger` with no action). Print usage for the
        # relevant parser and signal a usage error.
        if getattr(args, "command", None) == "ledger":
