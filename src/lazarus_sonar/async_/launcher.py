#!/usr/bin/env python3
"""LAZARUS v2 launcher (Stop / PostToolUse) -- NON-BLOCKING.
...
background runner (``lazarus_sonar.async_.runner`` via the
``hooks/async_runner_entry.py`` shim, or the ``lazarus-audit-bg`` console script
when installed), and RETURNS in single-digit milliseconds. It runs no SONAR and
no judge -- all of that latency now lives inside the detached child, off the
critical path (see the v2 canonical concurrency contract, sections 3 and
D-1..D-9).

Where this lives, and how it is invoked
---------------------------------------
The logic lives here, in the package (``lazarus_sonar.async_.launcher``), so it
is importable and testable like any other engine module. The thin
``hooks/async_launcher.py`` entrypoint is what Claude Code wires on
Stop / PostToolUse; it puts ``src/`` and ``hooks/`` on ``sys.path`` and calls
``main()`` here. That split keeps the hook file trivial and the logic under test.
...
Relationship to the v1 sync path (D-1, additive strategy):
  - The v1 engine is REUSED, never rewritten. This launcher imports v1's own
    work-unit extractor and fail/emit helpers from the ``retro_audit`` hook, so
    the async path parses hook payloads identically to the sync path -- the same
    event yields the same work-unit signature on both paths, and the ledger /
    pending keys line up.
...
Fail-loud vs fail-safe (D-9), re-applied for this hook:
  ...
  - An empty extraction, or a disabled async mode, is a QUIET non-blocking no-op
    ...
    path's prime directive is "never wedge a turn".
...
Latency contract:
  - File I/O + one ``subprocess.Popen`` + return. NO ``.wait()``, NO
    ``.communicate()``, NO PIPE this process reads. The child re-parents and
    outlives the hook.
"""
...
# Repo layout anchors.
#
# This module lives at src/lazarus_sonar/async_/launcher.py, so:
#   parents[0] = async_/         parents[1] = lazarus_sonar/
#   parents[2] = src/            parents[3] = repo root
# The detached runner is spawned via the checkout shim hooks/async_runner_entry.py
# (preferred, so it works with no install) or the lazarus-audit-bg console script.
# --------------------------------------------------------------------------- #
...
# The v1 extractor + fail/emit helpers live in hooks/retro_audit.py, a scripts
# dir, not part of the installed package. Add it to sys.path on import so the
# async path reuses the exact v1 extraction (D-3). A genuine "retro_audit is
# broken" ImportError still propagates -- we only bootstrap the path.
...
def _spawn_detached(argv: list[str], *, log_path: Path) -> None:
    ...
    Discipline (D-2):
      * NO `.wait()`, NO `.communicate()`, NO `.poll()` loop, NO PIPE this
        process reads. We construct the `Popen` and return.
      ...
      * Windows: `CREATE_NEW_PROCESS_GROUP | DETACHED_PROCESS`
        ...
        reaching it.
      * POSIX: `start_new_session=True` (setsid), so the child leaves the
...
def _child_argv(
    *,
    ...
    We prefer the checkout shim ``hooks/async_runner_entry.py`` (a small bootstrap
    that puts src/ on the path and calls ``lazarus_sonar.async_.runner.main()``),
    so the detached child works from a plain checkout with no install, mirroring
    the v1 hooks' import bootstrap. If that shim is absent (an installed layout
    without the hooks dir shipped), fall back to the ``lazarus-audit-bg`` console
    script, which the runner registers in pyproject ``[project.scripts]``.
    """
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    if not raw or not raw.strip():
        ...
        # is the one loud-on-empty case: it means the hook was invoked wrong.
        # Still exit non-blocking-safe via _fail_loud (exit 2 does not block
        # PostToolUse; Stop got no additionalContext).
