...
class PendingFinding:
    ...
    def from_retrofix(
        ...
        *,
        ...
        ``fix['path']`` is coerced to ``str`` here because ``RetroFix.as_dict()``
        carries ``path`` as a ``pathlib.Path`` (a ``WindowsPath`` on Windows),
        which is not JSON-serializable. ``to_json`` is a pure ``json.dumps`` with
        no ``default=str``, so without this coercion the SURFACED line could never
        be written (``TypeError``) and the whole async cycle would be red. The
        line-schema and this dataclass both already document ``fix.path`` as a
        ``str``, so normalizing it at the single point a RetroFix crosses into the
        queue keeps that on-disk invariant honored.
        """
        payload = fix.as_dict()
