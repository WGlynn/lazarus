...
That is the anti-drift property in DECISION D-1: there is exactly one place
that audits (``run_lazarus``), one signature function (``work_unit_signature``),
one ``Candidate`` type, one ``ScoringConfig``. v2 adds a transport, not a second
...
``lazarus_sonar.async_.runner``. This file is the runner delivered at
``src/lazarus_sonar/async_runner.py`` -- directly in the ``lazarus_sonar``
package rather than the ``async_`` subpackage -- so its imports of the v1 engine
are plain ``.``-sibling imports (``from .config import ...``), while the pending
queue and the vendored offline stub live in the ``.async_`` subpackage
(``from .async_.pending import ...``). The public surface, the
``run_background_audit`` body, and the ``main`` CLI are exactly as the contract
specifies; only the import anchors are adapted to this on-disk location. The v1
engine files are imported, never edited.

Console entrypoint
------------------
``main`` is wired as the ``lazarus-audit-bg`` console script (pyproject
``[project.scripts]``, alongside the existing ``lazarus``). It is what the
launcher spawns detached. It reads the work-unit from ONE of three mutually
exclusive sources -- a spool FILE (the launcher's channel, because by the time
the child reads it the parent hook has already exited and a stdin pipe would be
closed), stdin (manual/debug), or a raw Claude Code hook-event JSON reusing the
v1 extractor -- runs ``run_background_audit``, and exits with the runner's own
fail-loud codes: 0 on a clean audit (fixes may be 0), 2 on bad input / missing
config / missing corpus, 3 on a judge fault.

Fail-loud vs fail-safe (DECISION D-9)
-------------------------------------
The runner is the DETACHED CHILD. It is off the critical path, so "fail loud"
here means "inspectable in the child's stderr / the launcher's per-run log
file", not "on the user's console". A misconfig or bad input exits 2; a judge
fault exits 3 and simply yields no new pending lines this run -- it degrades to
...
-----------------------------------------------------
``judge_fn`` is the SAME ``lazarus.JudgeFn`` seam ``run_lazarus`` already
exposes. Passing ``--stub`` (or ``[async].stub_judge = true`` in config,
propagated by the launcher) injects the deterministic offline judge the v1 demo
ships, so the entire async cycle runs with NO ``anthropic`` package and NO
``ANTHROPIC_API_KEY``. From a checkout the runner prefers
``examples/demo/stub_judge.py`` (the source of truth); when installed it falls
back to the vendored ``lazarus_sonar.async_.stub_judge`` re-export.
...
#
# This file lives directly in the lazarus_sonar package, so the v1 engine
# modules are plain sibling imports. The pending queue and the vendored stub
# live in the async_ subpackage. Everything here is the exact same surface the
# v1 sync hook (hooks/retro_audit.py) and the demo (examples/demo/run_demo.py)
# import -- there is no second engine.
# --------------------------------------------------------------------------- #
...
from .async_.pending import PendingQueue, PendingFinding
...
def _import_extract_work_unit():
    ...
    # This file: src/lazarus_sonar/async_runner.py -> repo root is parents[2].
...
def _load_stub_judge_fn() -> JudgeFn:
    ...
    # Installed fallback: the vendored re-export inside the async_ subpackage.
    try:
        from .async_.stub_judge import stub_judge_fn  # type: ignore[import-not-found]
        return stub_judge_fn
    except ImportError as exc:
        _fail_loud(
